...
export default function VideoPage() {
  ...
  const fetchImages = async (page: string, pageSize: string) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/video/${id}?page=${page}&page_size=${pageSize}`)
      if (!response.ok) {
  ...
  const fetchTintLevel = async (imageId: string) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/tint/${imageId}`)
  ...
  const changePage = (newPage: number) => {
    router.push(`/video/${id}?page=${newPage}&page_size=${videoDetails?.pagination.page_size}`)
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200">
      <div className="container mx-auto p-4">
        <h1 className="text-4xl font-bold mb-6 text-gray-800 text-center py-8">Images from Video {id}</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {videoDetails.car_images.map((image: CarImage) => (
            <Card key={image.image_id} className="overflow-hidden bg-white shadow-md">
        ...
        <div className="flex flex-col items-center space-y-4">
          <div className="flex justify-center items-center space-x-2">
            <Button
              onClick={() => changePage(videoDetails.pagination.current_page - 1)}
            ...
            <span className="text-lg font-medium text-gray-700">
              Page {videoDetails.pagination.current_page} of {videoDetails.pagination.total_pages}
            </span>
            <Button
              onClick={() => changePage(videoDetails.pagination.current_page + 1)}
          ...
          <div className="text-sm text-gray-600">
            Showing {(videoDetails.pagination.current_page - 1) * videoDetails.pagination.page_size + 1} - {Math.min(videoDetails.pagination.current_page * videoDetails.pagination.page_size, videoDetails.pagination.total_images)} of {videoDetails.pagination.total_images} images
          </div>
        </div>
      </div>
    </div>
